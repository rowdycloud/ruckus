...
9. **Cleanup:** Remove the marker file `.ruckus/.migration-in-progress` regardless of whether `.ruckus/` is also removed. If `.ruckus/` is empty after removing the marker, remove the directory. If `.ruckus/` is still non-empty (user chose `leave` at step 8), do NOT remove `.ruckus/`; print: `"Files remain in .ruckus/. Move or delete them when ready."` and continue.
